...
<template>
  <section class="space-y-5">
    <AdminPageHeader :eyebrow="`项目 // ${isEdit ? '编辑' : '新建'} · SELECTED WORK`" :title="isEdit ? '编辑项目' : '新建项目'" description="维护公开案例目前真实可表达的字段；详情使用与公开端相同的 Markdown 渲染器。">
      <template #actions><div class="flex flex-wrap gap-2"><span class="admin-editor-state" :class="guard.isDirty ? 'is-dirty' : 'is-clean'"><i aria-hidden="true"></i>{{ guard.isDirty ? '有未保存修改' : '已保存' }}</span><RouterLink to="/admin/projects"><BaseButton variant="secondary" size="sm">返回项目</BaseButton></RouterLink></div></template>
    </AdminPageHeader>
