...
const groups = computed(() => {
  ...
  if (remaining.size) {
    matched.push({ key: 'other', label: '其他工具', hint: 'LEARNING / PRACTICE', patterns: [], technologies: [...remaining] });
  }
  return matched;
});
</script>
...
<template>
  <section class="relative left-1/2 w-[min(100vw-2rem,76rem)] -translate-x-1/2 py-16 sm:py-20 lg:py-28" aria-labelledby="tech-landscape-title">
    <div class="tech-shell">
      ...
      <p v-if="activeTech" class="tech-signal" aria-live="polite">当前选择：{{ activeTech }}，它正在这片花园里工作。</p>
    </div>
  </section>
</template>
